...
namespace MedicalSys.Framework
{
    ...
    public class MSRollingFileAppender : MSFileAppender
    {
        private static string EXTEND_NAME = ".log";
        private static int EXTEND_NAME_LENGTH = EXTEND_NAME.Length;
        ...
        /// </summary>
        /// <param name="baseFileName">filename of base</param>
        /// <returns>baseFileName</returns>
        private static string GetWildcardPatternForFile(string baseFileName)
        {
            if (baseFileName.EndsWith(EXTEND_NAME))
            {
                return baseFileName.Substring(0, baseFileName.Length - EXTEND_NAME_LENGTH) + "*";
            }
        ...
        /// pattern.
        /// </summary>
        /// <param name="baseFilePath">baseFilePath</param>
                ...
                if (Directory.Exists(directory))
                {
                    ...
                    if (files != null)
                    {
                        for (int i = 0; i < files.Length; i++)
                        {
                            ...
                            if (curFileName.StartsWith(GetBaseFileNameWithoutDotLog(baseFileName)))
                            {
        ...
        private string GetBaseFileNameWithoutDotLog(string baseFileName)
        {
            if (baseFileName.EndsWith(EXTEND_NAME))
            {
        ...
        private void RollOverIfDateBoundaryCrossing()
        {
            if (m_staticLogFileName && m_rollDate)
            {
                if (FileExists(m_baseFileName))
                {
                    ...
                    LogLog.Debug("RollingFileAppender: [" + last.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo) + "] vs. [" +
        ...
        /// </summary>
        /// <param name="baseFile">baseFile</param>
        /// <param name="curFileName">curFileName</param>
        private void InitializeFromOneFile(string baseFile, string curFileName)
        {
            if (!curFileName.StartsWith(GetBaseFileNameWithoutDotLog(baseFile)))
            {
                // This is not a log file, so ignore
                return;
            }
            if (curFileName.Equals(GetBaseFileNameWithoutDotLog(baseFile)))
            {
                // Base log file is not an incremented logfile (.1 or .2, etc)
                return;
            }

            if (curFileName.Length > GetBaseFileNameWithoutDotLog(baseFile).Length + 9)
            {
                // Base log file is not an incremented logfile (.1 or .2, etc)
                return;
            }

            int index = curFileName.IndexOf(".");//, curFileName.IndexOf(".") + 1);
            if (-1 == index)
            {
            ...
            if (m_staticLogFileName)
            {
                ...
                if (curFileName.LastIndexOf(".") == index)
                {
            ...
            try
            {
                ...
                int backup;
                if (SystemInfo.TryParse(curFileName.Substring(index + 1, curFileName.LastIndexOf(".") - index - 1), out backup))
                {
        ...
        /// </summary>
        /// <param name="baseFile">baseFile</param>
        /// <param name="arrayFiles">array of Files</param>
        ...
        /// </para>
        /// </remarks>
        protected void RollFile(string newFromFile, string newToFile)
        {
            string fromFile = newFromFile.Replace(EXTEND_NAME, "") + EXTEND_NAME; ;
            //if (!newFromFile.EndsWith(EXTEND_NAME))
            //{
            //    fromFile = newFromFile + EXTEND_NAME;
            //}

            string toFile = newToFile.Replace(EXTEND_NAME, "") + EXTEND_NAME;
